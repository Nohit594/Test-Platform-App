...
export default function TestPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        ...
        >
            {/* Header */}
            <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-3 sm:px-6 shrink-0 z-20">
                <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0 pr-2">
                    <h1 className="text-lg sm:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500 truncate">
                        {test.title}
                    </h1>
                </div>
                <div className="flex items-center gap-3 sm:gap-6 shrink-0">
                    <div className={cn("flex items-center gap-1.5 sm:gap-2 px-3 py-1 sm:px-4 sm:py-1.5 rounded-full border",
                        ...
                        <span className="font-mono text-base sm:text-lg font-bold">{formatTime(timeLeft)}</span>
                    </div>
                    <button
                        className="md:hidden p-1.5 sm:p-2 text-slate-400 hover:text-white transition-colors"
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <main className="flex-1 flex flex-col overflow-hidden relative">
                    {/* Question Content */}
                    <div
                        className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-10 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent select-none"
                    ...
                    >
                        <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
                            <div className="flex gap-3 sm:gap-4">
                                <span className="flex-shrink-0 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-800 border border-slate-700 text-cyan-400 font-bold text-sm sm:text-base">
                                    {currentQuestion.number}
                                </span>
                                <div className="text-base sm:text-lg md:text-xl leading-relaxed text-slate-200 pt-1">
                                    <LatexRenderer text={currentQuestion.text} />
                                </div>
                            </div>

                            <div className="space-y-3 pl-0 sm:pl-11 md:pl-14">
                                {currentOptions.map((opt, idx) => {
                                    ...
                                    return (
                                        <label
                                            key={idx}
                                            className={cn(
                                                "flex items-start gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl cursor-pointer transition-all border-2 group",
                                                isSelected
                                        ...
                                        >
                                            <div className={cn(
                                                "mt-0.5 sm:mt-1 w-4 h-4 sm:w-5 sm:h-5 shrink-0 rounded-full border-2 flex items-center justify-center transition-colors",
                                                isSelected ? "border-cyan-500" : "border-slate-600 group-hover:border-slate-500"
                                            )}>
                                                {isSelected && <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-cyan-500" />}
                                            </div>
                                            <span className="text-slate-300 text-base sm:text-lg leading-snug select-none">
                    ...
                    {/* Bottom Toolbar */}
                    <div className="h-16 sm:h-20 bg-slate-900 border-t border-slate-800 flex items-center justify-between px-2 sm:px-6 shrink-0 gap-2 overflow-x-auto scrollbar-none">
                        <div className="flex gap-1.5 sm:gap-3 shrink-0">
                            <button
                                onClick={handlePrevious}
                                disabled={currentQuestionIndex === 0}
                                className="flex items-center justify-center sm:justify-start gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg border border-slate-700 text-slate-300 hover:bg-slate-800 disabled:opacity-50 transition-colors h-9 sm:h-auto"
                            >
                            ...
                            <button
                                ...
                                className="px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg border border-slate-700 text-slate-300 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/50 transition-colors text-sm sm:text-base h-9 sm:h-auto"
                            >
                                Clear
                            </button>
                        </div>
                        ...
                        <div className="flex gap-1.5 sm:gap-3 shrink-0">
                            <button
                                ...
                                className="flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/50 text-yellow-400 hover:bg-yellow-500/20 transition-colors h-9 sm:h-auto"
                                title="Mark for Review & Next"
                            >
                                <Bookmark size={14} className="sm:w-[18px] sm:h-[18px]" />
                                <span className="hidden sm:inline">Mark for Review & Next</span>
                                <span className="sm:hidden text-xs font-medium">Review</span>
                            </button>
                            <button
                                onClick={handleSaveAndNext}
                                className="flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-6 py-1.5 sm:py-2 bg-cyan-600 hover:bg-cyan-500 text-white font-semibold rounded-lg shadow-lg shadow-cyan-500/20 transition-all h-9 sm:h-auto whitespace-nowrap"
                            >
